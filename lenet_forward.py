...
def softmax_classification(t, y, classes):
    ...
    prob_sort=X_prob.argsort(axis=1)

    predict_prob_indics = X_prob.argmax(axis=1)

    match = classes[predict_prob_indics] == y
   
    wrong_index= np.where(match == False)[0]

    wrong_class=prob_sort[wrong_index,-3::]

    return  wrong_index,wrong_class
...
#print(softmax_classification(X, y, classes))
...
class LeNet(chainer.Chain):
    ...
    def predict(self,x_data,y_data,train=False):
        ...
        w_indices,w_classes = softmax_classification(h.data,y_data,classes_)
        return w_indices,w_classes
